...
(IFEval-da 62.4 vs 56.1); it costs ScaLA-da and Nordjylland summarisation. Ranking of the
5k models: `max_reward ≈ simpo_full > generated > gold`.
...
1. **GRPO baseline (next).** Already tooled and queued. The paradigm fork: online RL on
   the Skywork RM vs the offline CroCo pipeline. Read its **training-reward trajectory**
   — if it climbs where offline reward-acc was flat, the offline pairs are the bottleneck.
2. **Recipe-quality fix at 5k.** One run: RM-margin filtering (or K≥8) + LR→1e-5 +
   2 epochs. Success = reward-acc > ~0.7 and a CI-clean win over `max_reward`. If it
   can't beat baseline at 5k, scaling won't help.
