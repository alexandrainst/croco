...
install: ## Install dependencies
	@echo "Installing the 'croco' project..."
	@$(MAKE) --quiet install-uv
	@$(MAKE) --quiet install-dependencies
	@$(MAKE) --quiet setup-environment-variables
	@$(MAKE) --quiet setup-git
	@$(MAKE) --quiet add-repo-to-git
	@echo "Installed the 'croco' project! You can now activate your virtual environment with 'source .venv/bin/activate'."
...
add-repo-to-git:
	...
	@if [ "$(shell git remote)" = "" ]; then \
		git remote add origin git@github.com:alexandrainst/croco.git; \
	fi
...
docker:  ## Build Docker image and run container
	@make install
	@docker build -t croco .
	@docker run -it --rm croco
